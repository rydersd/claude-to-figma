...
export function registerTools(server: McpServer, sendCommandToFigma: SendCommandFn) {
  server.tool(
    "figma_eval",
    "Execute Figma Plugin API code in the plugin sandbox. Code is the body of an async function receiving: figma, hexToFigmaColor, appendOrInsertChild, loadAllFonts, getVariableByName, bindVariableToColor, resolveColorValue, sendProgressUpdate, introspectNode, setProperties. Use `return` to send results. Figma nodes auto-serialize to {id, name, type}.",
    {
    ...
    async ({ code }: { code: string }) => {
      try {
        ...
        const typedResult = result as { success: boolean; result?: any; error?: string; stack?: string };
        if (typedResult.success) {
          return { content: [{ type: "text" as const, text: JSON.stringify(typedResult.result, null, 2) }] };
        } else {
          return { content: [{ type: "text" as const, text: `Eval error: ${typedResult.error}${typedResult.stack ? "\n\n" + typedResult.stack : ""}` }] };
        }
      } catch (error) {
